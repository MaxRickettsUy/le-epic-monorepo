...
| --------------------------------- | ------------ | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /band/`                      | —            | `BandList`                          | `?page=` (1-based), `bands_per_page` from backend settings. `?sort=` is `name` (default) or `recent` (created_at desc). Optional facets, combinable: `?genre=<slug>`, `?country=<exact>`, `?letter=<A–Z or #>` (`#` = name not starting A–Z; other values → `422`). Auto-flagged bands are hidden until a curator allowlists or deletes them. The pagination total reflects the active facets. |
| `GET /band/countries`             | —            | `CountryCount[]`                    | Distinct band countries with counts, desc by count then name (auto-flagged bands excluded). Free-text column (no normalization). Used to populate the country browse facet.                                                                                                                                                                                                                    |
| `GET /band/needs-review`          | —            | `NeedsReviewItem[]`                 | Auto-flagged bands awaiting curator review, lowest `seed_share` first. Default filter: `auto_flagged IS TRUE AND allowlisted_at IS NULL`. `?include_resolved=true` widens it to previously allowlisted bands too (so curators can undo); `?limit=` (default 50, max 200). Carries the raw `mb_tags` snapshot inline. Backs `/admin/needs-review`.                                              |
...
| `POST /band/{id}/allowlist`       | —            | `{ message, id }`                   | Curator override for an auto-flagged band: clears `auto_flagged` and stamps `allowlisted_at`. Sticky — `seed.mb_dump` skips re-flagging when `allowlisted_at` is set. `400` if band not awaiting review; `404` if missing.                                                                                                                                                                     |
| `DELETE /band/{id}/delete`        | —            | `"band deleted"`                    | `?blacklist=true` (default) records the band's MBID in the `band_blacklist` table AND appends it to `backend/seed/blacklist.json` so the decision survives DB resets / propagates to teammates once you commit the file diff. Pass `?blacklist=false` to opt out, or `?reason=` to attach a curator note. Bands without an MBID are never blacklisted.                                         |
...
```json
{
  ...
  allowlisted_at?: string; // ISO timestamp set by POST /band/{id}/allowlist; null = never reviewed
}
```
