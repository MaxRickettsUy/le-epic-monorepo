...
| `POST /band/{id}/allowlist`       | —            | `{ message, id }`                   | Curator override for an auto-flagged band: clears `auto_flagged` and stamps `allowlisted_at`. Sticky — `seed.mb_dump` skips re-flagging when `allowlisted_at` is set. `404` if missing.                                                                                                                                                                                                        |
