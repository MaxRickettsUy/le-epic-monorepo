...
| `DELETE /band/{id}/delete`        | —            | `"band deleted"`                    | `?blacklist=true` (default) records the band's MBID in `band_blacklist` so `seed.mb_dump` will not re-add it on the next run; pass `?blacklist=false` to opt out, or `?reason=` to attach a curator note. Bands without an MBID are never blacklisted. Not used by the frontend yet.                                  |
