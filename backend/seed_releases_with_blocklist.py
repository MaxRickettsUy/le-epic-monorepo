...
with app.app_context():
    ...
    # ─── Prepare block list ───────────────────────────────────────────────────────
    block_list = []  # we'll append {'mbid': ..., 'name': ...} for non-punk bands
    ...
    for band in bands:
        ...
        if releases_added == 0:
            # No releases matched "hardcore punk" for this band → add to block list
            print(f"  ⚠️  No qualifying releases found for '{band.name}'. Adding to block list.")
            block_list.append({
                "mbid": mbid,
                "name": band.name
            })
            # (Optional) If you still want to delete from DB, keep these lines;
    ...
    # ─── 6) Dump block list to a file ─────────────────────────────────────────────
    if block_list:
        try:
            with open("data/block_list.json", "w", encoding="utf-8") as f:
                json.dump(block_list, f, indent=2, ensure_ascii=False)
            print(f"\n✅ Wrote {len(block_list)} blocked band(s) to block_list.json")
        except IOError as e:
            print(f"\n❌ Failed to write block_list.json: {e}")
    else:
        print("\n✅ No bands to block; block_list.json not created.")
